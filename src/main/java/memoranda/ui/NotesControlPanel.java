...
public class NotesControlPanel extends JPanel {
    ...
    void jbInit() throws Exception {
        tabbedPane.setFont(new java.awt.Font("Dialog", Font.BOLD, 10));
        ...
        ppOpenB.addActionListener(e -> ppOpenB_actionPerformed(e));
        ppOpenB.setIcon(
                new ImageIcon(
                        Objects.requireNonNull(AppFrame.class.getResource("/ui/icons/nopen.png"))));
        ...
        ppAddBkmrk.setFont(new java.awt.Font("Dialog", Font.BOLD, 11));
        ppAddBkmrk.setText(Local.getString("Set bookmark"));
        ppAddBkmrk.addActionListener(e -> ppAddBkmrk_actionPerformed(e));
        ppAddBkmrk.setIcon(
                new ImageIcon(Objects.requireNonNull(AppFrame.class
                        .getResource("/ui/icons/addbookmark.png"))));
        ppClearNote.setFont(new java.awt.Font("Dialog", Font.BOLD, 11));
        ppClearNote.setText(Local.getString("Clear note"));
        ppClearNote.addActionListener(e -> ppClearNote_actionPerformed(e));
        ppClearNote.setIcon(
                new ImageIcon(Objects.requireNonNull(AppFrame.class
                        .getResource("/ui/icons/editdelete.png"))));
        ppClearNote.setEnabled(false);
        notesPPMenu.setFont(new java.awt.Font("Dialog", Font.BOLD, 10));
        ppOpenNote.setFont(new java.awt.Font("Dialog", Font.BOLD, 11));
        ppOpenNote.setText(Local.getString("Go to note"));
        ppOpenNote.addActionListener(e -> ppOpenNote_actionPerformed(e));
        ppOpenNote.setEnabled(false);

        ppInvertSort.setFont(new java.awt.Font("Dialog", Font.BOLD, 11));
        ppInvertSort.setText(Local.getString("Invert Sort Order"));
        ppInvertSort.addActionListener(e -> ppInvertSort_actionPerformed(e));
        ppInvertSort.setEnabled(true);
        boolean descSort =
                (Configuration.get("NOTES_SORT_ORDER").equals("true"));
        ppInvertSort.setSelected(descSort);

        ppRemoveBkmrk.setFont(new java.awt.Font("Dialog", Font.BOLD, 11));
        ppRemoveBkmrk.setText(Local.getString("Remove bookmark"));
        ppRemoveBkmrk.addActionListener(e -> ppRemoveBkmrk_actionPerformed(e));
        ppRemoveBkmrk.setIcon(
                new ImageIcon(Objects.requireNonNull(AppFrame.class
                        .getResource("/ui/icons/removebookmark.png"))));
    ...
    void ppClearNote_actionPerformed(ActionEvent e) {
        ...
        if (notesList.getSelectedIndices().length > 1) {
            ...
            msg =
                    Local.getString("Clear note")
                            + "\n'"
                            + ((Note) notesList.getNote(notesList.getSelectedIndex())).getDate()
                            .getFullDateString()
                            + "'\n"
                            + Local.getString("Are you sure?");
        }
        ...
        for (int i = 0; i < notesList.getSelectedIndices().length; i++) {
            Note note = (Note) notesList.getNote(notesList.getSelectedIndices()[i]);
            if (CurrentProject.getNoteList().getActiveNote() != null && note.getDate()
                    .equals(CurrentProject.getNoteList().getActiveNote().getDate())) {
                /*Debug*/
...
//        ((AppFrame)App.getFrame()).workPanel.dailyItemsPanel.editorPanel.editor.requestFocus();
    ...
    void ppRemoveBkmrk_actionPerformed(ActionEvent e) {
        ...
        notesList.updateUI();
        notesList.clearSelection();
        ((AppFrame) App.getFrame()).getDailyItemsPanel().editorPanel.editor.requestFocus();
    }

    void ppSetEnabled() {
        boolean enbl = (notesList.getModel().getSize() > 0) && (notesList.getSelectedIndex() > -1);

        ppRemoveBkmrk.setEnabled(
                enbl && (((Note) notesList.getNote(notesList.getSelectedIndex())).isMarked())
                        || notesList.getSelectedIndices().length > 1);
        ppAddBkmrk.setEnabled(
                enbl && !(((Note) notesList.getNote(notesList.getSelectedIndex())).isMarked())
                        || notesList.getSelectedIndices().length > 1);
